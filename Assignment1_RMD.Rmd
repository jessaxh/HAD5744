...
What raises concern with the outputs of the table is the large standard deviation and standard error for GDPPC and HXPC. This indicates that large spread of data around the mean and that the sample mean is not a good representation of the populaiton.
...
#the third_regression is the regression of LEBF on log(HXPC) + log(GDDPC)
...
#the bptest is showing that the we cannot reject the null that the residuals are distributed with equal variance (homoscedasticty) as there is a low test statistic (BP = 4.8241) and a p value > 0.05, therefore we assume that the residuals are homoscedastic
#https://www.codingprof.com/3-easy-ways-to-test-for-heteroscedasticity-in-r-examples/#:~:text=In%20R%2C%20the%20easiest%20way,Test%20or%20the%20White%20Test.


#test for autocorrelation
lmtest::dwtest(third_regression)

#a test for autocorrelation showed that there is no autocorrelation among the residuals either as the p value is greater than 0.05 and the test statistic for the Durbin-Watson test is close to 2. 
#https://www.codingprof.com/3-easy-ways-to-test-for-autocorrelation-in-r-examples/?fbclid=IwAR0vNkTqYZcCfr9z-5JfsaLAGNColblMiHqXQoSS4PnmBu_egLh5u3j0r3I #:~:text=In%20R%2C%20the%20easiest%20way,or%20the%20Breusch%2DGodfrey%20test

```

The results from quesiton 9, and our intuition suggest that the standard errors in our specification are not homoscedastic. To check if it is accurate to use homoscedastic errors, a plot was created to look at the residual vs fitted values of our model. In this residual plot, we can see that the data is not randomly spread around the 0 line and thus indicative that the use of homoscedastic standard errors may not be best practice. As a result of the results seen in this plot, we tried to implement both a correction for heteroskedasticity and autocorrelation within our model. 

When we used the heteroskedasticity-robust method for our standard errors we observed the standard errors for log(HXPC) and log(GDPPC) increased from 1.54 and 1.66 to 2.19 and 2.39 respectively. The standard errors increasing indicates that our standard errors are potentially becoming more accurate however the variation of the data on the regression line is quite large. 

When we used the Neweywest method to try and adjust for autocorrelation in our standard errors we observed similar results to that of the heteroskedasticity model. In this model the standard errors for log(HXPC) and log(GDPPC) increased from 1.54 and 1.66 to 1.87 and 2.03. Again the standard errors increased and thus potentially indicate that we have a more accurate representation of the variability around our regression line. 

To confirm if adjusting our standard errors for heteroskedasticity or autocorrelation was the right strategy a Breusch-Pagan test and a Durbin-Watson test were conducted. The Breusch-Pagan test results, showed that we cannot reject the null that the residuals are distributed with equal variance (homoscedasticty). The Durbin-Watson test results, showed that there is no autocorrelation present among the residuals in our model. As a result, both methods to correct for the standard errors in our inital model were not as effective as we hoped they would be. We can conclude that although the results of our specifications with both a heteroskedastic-robust correction and Neweywest correction had an effect on our standard errors, they are not the right corrections to account for the variance we observed in our residual-fitted plot. 
