...
se_corrections<- msummary(list("homoscedastic" = third_regression,
          ...
          stars=c('*' = .1, '**' = .05, '***' = .01), vcov =c("classical","robust"),
          fmt = "%.4f")
...
To confirm if adjusting our standard errors for heteroskedasticity was the right strategy a Breusch-Pagan test was conducted. The Breusch-Pagan test results, showed that we cannot reject the null that the residuals are distributed with equal variance (homoscedasticty). As a result, the method to correct for the standard errors in our inital model was not as effective as we hoped it would be. We can conclude that although the results of our specification with  a heteroskedastic-robust correction had an effect on our standard errors, they are not the right corrections to account for the variance we observed in our residual-fitted plot. 
