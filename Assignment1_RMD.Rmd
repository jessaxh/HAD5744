...
```
Our DAG does not indicate that we can interpret any regression coefficients causally.
...
Regression_table1
```
A one unit increase in health expenditure per capita is significantly associated with a 0.004 increase in life expectancy at birth for females.
...
```{r fourth regression pt 2}
fourth_regression <- lm(LEBF20052~log(HXPC2005)+log(GDPPCUS2005), Dataset1)
Regression_table4 <- msummary(list(third_regression, fourth_regression), 
                              stars=c('*' = .1, '**' = .05, '***' = .01),
                              fmt = "%.4f")
Regression_table4
```
...
When we log transform our model, we see a change in direction for the relationship between HXPC and LEBF (when holding GDPPC constant), however, the relationship continues to be not statistically significant. For the log-transformed relationship between GDPPC and LEBF (when keeping HXPC constant), we continue to see a positive significant relationship; however, a 10% increase in GDDPC is significantly associated with a 4.1144 increase in LEBF which is a dramatic increase from the non-transformed model where GDDPC was significantly associated with a 0.0005 increase in LEBF.
...
#used^ code to know how to get continents from countries
library(countrycode)
...
Regression_table5 <- msummary(continet_regression,
                              stars=c('*' = .1, '**' = .05, '***' = .01),
                               fmt = 5)
Regression_table5
...
Regression_table6 <- msummary(interaction_regression,
                              stars=c('*' = .1, '**' = .05, '***' = .01),
                              fmt = 5)
