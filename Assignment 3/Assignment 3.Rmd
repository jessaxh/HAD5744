...
# Question 3b
```{r Question 3b}
naive_ols <- lm(log(totexp) ~ medicare + sex + educyr + racex + faminc, data = mydata, weights = perwt) # We will have to figure out what we want to control for
summary(naive_ols)
...
quant_reg <- rq(totexp ~ medicare + educyear + sex + racex + faminc, tau= c(0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75,
...
quantreg <- rq(totexp ~ medicare + educyr + sex + racex + faminc, data = mydata, weights = perwt,
  ...
  quantsummary <- summary(quantreg, se = "boot")
