...
#One thing that does concern us is the large negative coefficient associated with the pandemic_begun variable for the All control groups model. The coefficient of -8.053 which indicates that if all other regressors were held constant there is a large decrease in popularity when the pandemic began which is an interesting result. When looking for control groups we would ideally want the control group and the treatment group to have similar prior trends and with all things held constant nothing to change into the future. When the model drops the soup and sandwich control groups and the coefficient for pandemic_begun is looked at again we can see that the coefficient is now -0.493. This indicates that just having the pandemic begin with all other regressors held constant the treatment and control group are more similar. This confirms that dropping soup and sandwich is the right decision for moving forward. 
...
#When we drop soup and sandwich we aslo see the the regression coefficient for the interaction between being the treated variable and the pandemic begining decreases indicating the increased popularity effect is more true to what is really going on. 
...
#hey aidan how'd we get 26 times?
...
```
The difference between the two curves begins to converge as family income increases. We can see at the lower end of family income (approaching zero) that the differences in total expenditure are greatest between people who receive and do not receive medicare. 
#When family income increases the curves begin to converge which suggests that regardless of medicare coverage people who earn more will incur less cost on the healthcare system. 
This also suggests that as lower income individuals will require more healthcare services, being enrolled in medicare can be very beneficial in ensuring access to care. Between the two curves it does appear that the difference in expenditure between the medicare group and non-medicare group is related to family income. As family income increases for both groups they each have a decrease in expenditure and this indicates that higher family incomes lead to health family members and less expenditure on health care. 
